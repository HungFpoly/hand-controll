const _settings = {
  ...
  modelURL: "assets/lowpoly_humming-bird_animated.glb",
...
function trigger_poppingEffect() {
  ...
  const scaleBegin =
    _three.poppingObject.scale.x === _settings.scale
      ? 0.01
      : _three.poppingObject.scale.x;
  ...
  // shadow opacity:
  const tweenShadowAppear = new TWEEN.Tween(
    blobShadowUniforms.blobShadowAlphaMax
  )
    .to(
      {
  ...
  const tweenRealColors = new TWEEN.Tween(
    hologramUniforms.hologramTransitionColorCoeff
  )
    .delay(colorDelay)
    .to(
      {
  ...
  const tweenShadowColor = new TWEEN.Tween(
    blobShadowUniforms.blobShadowColor.value
  )
    .delay(colorDelay)
    .to(blobShadowColor, colorDuration)
...
function trigger_unpoppingEffect() {
  ...
  const duration = 500;
  const tweenHologramDisappear = new TWEEN.Tween(
    hologramUniforms.hologramAlphaSrc
  )
    .to(
      {
        value: 0,
  ...
  const tweenHologramColors = new TWEEN.Tween(
    hologramUniforms.hologramTransitionColorCoeff
  )
    .to(
      {
  ...
  const tweenShadowDisappear = new TWEEN.Tween(
    blobShadowUniforms.blobShadowAlphaMax
  )
    .to(
      {
...
function create_blobShadow() {
  ...
  const blobShadowUniforms = {
    blobShadowRadiusRange: {
      value: new THREE.Vector2(
        _settings.blobShadowRadiusStart,
        1
      ).multiplyScalar(_settings.blobShadowSize * 0.5),
    },
    blobShadowColor: {
  ...
  const blobShadowMat = new THREE.ShaderMaterial({
    ...
    blending: _settings.blobShadowAdditiveBlending
